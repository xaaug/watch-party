...
const Rooms: React.FC<Props> = ({fetchData}) => {
  ...
  return (
    <>
    <p className="mt-6 text-2xl font-semibold font-mono ">Hello {user?.displayName?.split(' ')[0]}👋</p>
    <div className="mt-6">
      <ExpandableTile
      ...
      >
        ...
        <TileBelowTheFoldContent>
          {userRooms.length > 0 ? <div className="mt-8 mb-12 flex flex-col gap-2">
            {userRooms.map((room) => (
          ...
          </div> : <div className="flex flex-col gap-6 mt-6">
    ...
    <div className="my-6">
      ...
      >
        ...
        <TileBelowTheFoldContent>
          {roomsIn.length > 0 ? <div className="mt-8 mb-12 flex flex-col gap-2">
            {roomsIn.map((room) => (
                <Link to={`/room/${room.roomId}`}>
              <Tile>
                <p>{room.roomId}</p>
